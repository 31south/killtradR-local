...
async def fetch_real_market_slice(settings):
    ...
    try:
        ...
        errors.append(f"blofin: {repr(exc)}")
    finally:
        await blofin.close()
    ...
    try:
        ...
        errors.append(f"binance: {repr(exc)}")
...
async def main() -> None:
    ...
    bus = EventBus()
    detectors = [LiquidityGrabDetector(settings, bus), StopHuntDetector(settings, bus), OrderBookImbalanceDetector(settings, bus)]
    try:
        ...
        result = {"ok": True, "source": source, "candles_processed": len(candles), "order_books_processed": 1, "events_emitted": len(events), "events": events, "source_errors_before_success": source_errors}
    except Exception as exc:
