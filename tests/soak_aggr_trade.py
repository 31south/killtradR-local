...
async def main() -> None:
    ...
    try:
        async with websockets.connect(settings.aggr_trade_ws_url, ping_interval=20) as websocket:
            await websocket.send(json.dumps({"op": "subscribe", "channel": "orderbook", "symbol": settings.symbol}))
        ...
        result = {"ok": False, "url": settings.aggr_trade_ws_url, "error": repr(exc), "traceback": traceback.format_exc()}
